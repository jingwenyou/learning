        ...
        try:
            ...
            try:
                if resp.headers.get("Content-Type", "").startswith("application/json"):
                    log_util.info(f'{resp}\t{resp.json()}')
                else:
                    # 对于非JSON响应，限制日志长度
                    content_preview = self._format_data_for_log(resp.content)
                    log_util.info(f'{resp}\t{content_preview}')
...
class Request_tools(CustomSession):
    ...
    def get_gmcert_chain(self, cert_type: Literal[0, 1, 2, 3], alg: Literal['SM2', 'ECC', 'RSA']) -> Optional[bytes]:
        ...
        # 配置请求头
        headers = {
            "Cookie": "Hm_lvt_c01e2f6d5309271f7891bcf9b28b4505=1756798310,1757054860,1757494455,1758091503; "
            "HMACCOUNT=B44486ADA0F8752A; JSESSIONID=56379AFE0D421FBB99E9F0384B5B8912; "
            "Hm_lpvt_c01e2f6d5309271f7891bcf9b28b4505=1758611332",
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) "
            "Chrome/102.0.9999.0 Safari/537.36 htbrowser",
...
if __name__ == '__main__':
    req = Request_tools()
    print(req.get_gmcert_chain(0, 'SM2'))
